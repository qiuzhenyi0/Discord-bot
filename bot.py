...
async def update_sheet_record(interaction: discord.Interaction, name: str, item_label: str):
    if not sheet:
        await interaction.response.send_message("❌ 機器人目前未連線至試算表，請聯絡管理員。", ephemeral=True)
        return
...
@bot.tree.command(name="帶人", description="登記帶人項目")
@is_staff_or_admin()
@app_commands.choices(項目=[
...
@bot.tree.command(name="陪玩", description="登記陪玩項目")
@is_staff_or_admin()
@app_commands.choices(項目=[
...
@bot.tree.command(name="三戀", description="登記三戀項目")
@is_staff_or_admin()
@app_commands.choices(項目=[
...
@bot.tree.command(name="刪除", description="清除特定記錄 (管理員/特定身分組)")
@is_staff_or_admin()
async def delete_record(interaction: discord.Interaction, 名字: str, 項目名稱: str):
    ...
    try:
        ...
        if 名字 not in names_col or 項目名稱 not in header_row:
            await interaction.followup.send("❌ 找不到該玩家或項目，請確認名稱是否精準。")
            return
        ...
        await interaction.followup.send(f"✅ 已成功清除 **{名字}** 的 **{項目名稱}** 記錄！")
    except Exception as e:
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingAnyRole):
        ...
        if not interaction.response.is_done():
            ...
            await interaction.followup.send(f"⚠️ 系統錯誤：{error}", ephemeral=True)
